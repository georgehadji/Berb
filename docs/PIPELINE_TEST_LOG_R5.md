...
- **严重度**: 🟡 中 — 不阻塞但影响代码质量
- **描述**: `berb/agents/code_searcher/query_gen.py:149` 调用 `llm.chat()` 时传入不支持的 `user` 关键字参数
  ```
...
- **文件**: `berb/agents/code_searcher/query_gen.py:149`
- **描述**:
...
- **修复**: 在 `berb/templates/conference.py` 的 NEURIPS_2024、NEURIPS_2025、GENERIC 三个模板中
